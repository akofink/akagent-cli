...
func TestIntegrationLaunchIsRemoved(t *testing.T) {
	setupTaskCommandTest(t)
	result := runCommand(t, []string{"integration", "launch", "task"})
	if result.code != 2 || !strings.Contains(result.stdout, "integration launch") || !strings.Contains(result.stdout, "category: usage") {
		t.Fatalf("integration launch = (%d, %q), want migration error", result.code, result.stdout)
	}
}
